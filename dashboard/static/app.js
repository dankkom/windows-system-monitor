...
async function fetchJSON(url){const r=await fetch(url,{headers:{'Cache-Control':'no-store'}});if(!r.ok){let message=`HTTP ${r.status}`;try{message=(await r.json()).error||message;}catch{}throw new Error(message);}return r.json();}

async function loadHeader(){
  try{const [sys,db]=await Promise.all([fetchJSON('/api/system'),fetchJSON('/api/db_size')]);if(sys&&db){els.header.innerHTML=`<span><strong>${escapeHTML(sys.hostname||'')}</strong></span><span>up ${Math.floor((sys.uptime||0)/3600)}h</span><span>${sys.ram_gb?sys.ram_gb.toFixed(1)+' GiB RAM':''}</span><span>DB ${escapeHTML(db.size)}</span>`;els.footerDb.textContent=`${db.size} · ${db.cpu_rows} CPU · ${db.sensor_rows} sensores`;}}catch{}
}
...
async function updateOverview(){
  const win=encodeURIComponent(S.win);const [cpu,mem,gpu,disk,net,power,hb]=await Promise.all([fetchJSON(`/api/cpu?window=${win}`),fetchJSON(`/api/memory?window=${win}`),fetchJSON(`/api/gpu?window=${win}`),fetchJSON('/api/disk/usage'),fetchJSON('/api/net/latest'),fetchJSON(`/api/power?window=${win}`),fetchJSON('/api/heartbeat')]);
  const latest=(a,key)=>a.length?a[a.length-1][key]:null;const stale=hb.filter(r=>!r.success||Date.now()-new Date(r.ts)>180000).length;
  document.getElementById('ov-cards').innerHTML=card('CPU',fmtNum(latest(cpu,'cpu'),1,'%'))+card('Memória',fmtNum(latest(mem,'used_percent'),1,'%'))+card('Potência estimada',power.series.length?fmtNum(power.series.at(-1).estimated_w,1,' W'):'—')+card('Energia na janela',fmtWh(power.summary.estimated_wh))+card('Cobertura energia',power.meta.coverage_percent.toFixed(1)+'%',power.meta.coverage_percent<80?'warn':'')+card('Coletores com alerta',String(stale),stale?'bad':'');
...
async function updateCpu(){const win=encodeURIComponent(S.win);const [cpu,temps]=await Promise.all([fetchJSON(`/api/cpu?window=${win}`),fetchJSON(`/api/sensors/cpu_temps?window=${win}`)]);upsertChart('cpu-cpu',cpu.map(r=>r.ts),[line('CPU %',cpu.map(r=>r.cpu),COLORS[0])]);upsertChart('cpu-freq',cpu.map(r=>r.ts),[line('MHz',cpu.map(r=>r.freq),COLORS[3])]);const box=document.getElementById('cpu-temps');const names=[...new Set(temps.map(r=>r.name))].slice(0,4);for(let i=0;i<4;i++){if(S.charts[`cpu-temp-${i}`]){S.charts[`cpu-temp-${i}`].destroy();delete S.charts[`cpu-temp-${i}`];}}box.innerHTML=names.map((name,i)=>metric(name,`cpu-temp-${i}`,'temperature')).join('');names.forEach((name,i)=>{const rows=temps.filter(r=>r.name===name);upsertChart(`cpu-temp-${i}`,rows.map(r=>r.ts),[line('°C',rows.map(r=>r.value),COLORS[2])]);});}
...
async function updatePower(){
  ...
  const periods=['daily','weekly','monthly'].flatMap(type=>data.periods[type].map(r=>({tipo:{daily:'Dia',weekly:'Semana',monthly:'Mês'}[type],periodo:r.period,medida:fmtWh(r.measured_wh),estimada:fmtWh(r.estimated_wh)})));
  document.getElementById('power-periods').innerHTML=tableHTML(periods,[['tipo','Período'],['periodo','Referência'],['medida','Medido parcial'],['estimada','Estimado']]);
...
async function updateDisk(){
  const win=encodeURIComponent(S.win);const [usage,history,physical,smart,smartHistory,io]=await Promise.all([fetchJSON('/api/disk/usage'),fetchJSON(`/api/disk/usage/history?window=${win}`),fetchJSON('/api/disk/physical'),fetchJSON('/api/disk/smart'),fetchJSON(`/api/disk/smart/history?window=${win}`),fetchJSON(`/api/disk/io?window=${win}`)]);
  ...
  S.diskVolume=setOptions('disk-volume',history.map(r=>r.device),S.diskVolume,v=>{S.diskVolume=v;updateDisk();});const volumeRows=history.filter(r=>r.device===S.diskVolume);upsertChart('disk-capacity',volumeRows.map(r=>r.ts),[line('Usado GiB',volumeRows.map(r=>r.used_bytes/1073741824),COLORS[0]),line('Livre GiB',volumeRows.map(r=>r.free_bytes/1073741824),COLORS[1])]);
  ...
  upsertChart('disk-throughput',labels,[line('Leitura MiB/s',d.map(r=>r.read_bps/1048576),COLORS[0]),line('Escrita MiB/s',d.map(r=>r.write_bps/1048576),COLORS[1])]);
  upsertChart('disk-cumulative',labels,[line('Lido',d.map(r=>r.read/1073741824),COLORS[0]),line('Escrito',d.map(r=>r.write/1073741824),COLORS[1])]);
...
async function updateNet(){
  const win=encodeURIComponent(S.win);const [latest,hist]=await Promise.all([fetchJSON('/api/net/latest'),fetchJSON(`/api/net?window=${win}`)]);
  ...
  const preferred=hist.some(r=>r.iface==='Wi-Fi')?'Wi-Fi':S.netIface;S.netIface=setOptions('net-iface',hist.map(r=>r.iface),preferred,v=>{S.netIface=v;updateNet();});const rows=hist.filter(r=>r.iface===S.netIface),labels=rows.map(r=>r.ts),last=rows.at(-1);
  ...
  upsertChart('net-throughput',labels,[line('RX Mbit/s',rows.map(r=>r.recv_bps*8/1e6),COLORS[0]),line('TX Mbit/s',rows.map(r=>r.sent_bps*8/1e6),COLORS[1])]);
  upsertChart('net-cumulative',labels,[line('Recebido GiB',rows.map(r=>r.recv/1073741824),COLORS[0]),line('Enviado GiB',rows.map(r=>r.sent/1073741824),COLORS[1])]);
...
async function updateSystem(){const [sys,hb]=await Promise.all([fetchJSON('/api/system'),fetchJSON('/api/heartbeat')]);document.getElementById('sys-pre').textContent=sys?JSON.stringify(sys,null,2):'sem dados';document.getElementById('hb-table').innerHTML=tableHTML(hb.map(r=>({coletor:r.collector,estado:r.success?'OK':'FALHA',idade:Math.max(0,Math.round((Date.now()-new Date(r.ts))/1000))+' s',erro:r.error||''})),[['coletor','Coletor'],['estado','Estado'],['idade','Idade'],['erro','Erro']]);}
